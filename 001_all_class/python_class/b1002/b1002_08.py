...
if fruit.count(search)>0:
  ...
  for i in range(fruit.count(search)):
    print(f"{search} 글자가 있는 위치 :", fruit.find(search,idx)) # find(찾을문자, 시작index,끝index)
...
# fruit = "사과,배,딸기,포도,복숭아,배,사과,배,딸기"
# # 과일이름을 입력받아, 있으면 있다, 과일검색개수 :
# # 없으면 없습니다.
# search = input("과일이름을 입력하세요.")
# if search in fruit:
#   print("과일검색개수 :", fruit.count(search))
...
# search = input("과일이름을 입력하세요.")
# if search in fruit:
#   print(f"{search}가 있어요.")
# else:
#   print(f"{search}가 없어요.")
